...
```{r}
md_senior_opioids_working2 <- medicare_opioids %>%
  clean_names() %>%
  remove_empty_rows() %>%
  ...
  mutate(medicare_urban_opioid_prescribing_rate = na_if(medicare_urban_opioid_prescribing_rate, "ZERO")) %>%
  ...
  mutate(medicare_opioid_prescribing_rate = as.numeric(medicare_opioid_prescribing_rate),
         medicare_urban_opioid_prescribing_rate = as.numeric(medicare_urban_opioid_prescribing_rate),           medicare_rural_opioid_prescribing_rate = as.numeric(medicare_rural_opioid_prescribing_rate))     %>% 
  separate(county, into=c("county","state"), sep=",") %>%
  mutate(state = str_trim(state)) %>%
  mutate(county = tolower(county)) %>%
