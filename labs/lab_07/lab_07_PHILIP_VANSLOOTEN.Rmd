...
# load the suite of packages below:
library(tidyverse)
library(janitor)
library(arcos)
library(tidycensus)
...
```{r}
# Patterns in pills table (but not in the population table): There are counties in Arkansas, Connecticut, Georgia and a few other states with either a missing county FIPS code or county name. 
# Potential problems: Pill data for those states by population will be inaccurate due to missing info.
```
...
```{r}
# Define API Key
census_api_key("cbfec00858eabe7be8f5f2e381d263577de3e965")
...
#county_geodata <- get_acs(geography = "county", variables = "B01001_001", geometry = TRUE)

state_geodata <- get_acs(geography = "state", variables = "B01001_001", geometry = TRUE)
...
#I might not have enough memory on my Mac to run this. Not sure what the problem with mapview is.
```
...
geo_pills_population_state_2009  %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills Per Person',title="West Virginia Received Most Pills in 2009; Illinois Among Lowest", subtitle = "2009 ARCOS Opioid Data ", caption = "Source: Washington Post") +
...
geo_pills_population_state %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="Opioid Distribution Peaks in 2011", subtitle = "2014 per person levels still high despite slight decline", caption = "Source: ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)
...
# I commented all of this out because mapview wasn't working. This is just to show the process.

# Step 1: Create pills_population_county object
...
# Step 2: Join it to the shapefile data using GEOID

#geo_pills_population_county_2009 <- state_geodata_shifted %>%
 #inner_join(pills_population_county, by=c("GEOID" = "countyfips"))


#Step 3: Create the interactive map using the mapview()function

#mapview(geo_pills_population_county_2009, zcol='pills_per_person', legend = TRUE)

#TRENDS: Based on the map in the video, not only were counties in the southeast, such as Mingo, WV and a county in what appeared to be Florida, flooded with opioids in 2009 (the "opioid corridor"), but the south west around CA looked severely impacted as well. I wonder if there is a correlation with the current homeless crisis in that area.
```
...
```{r}
#I had a problem loading the census county datafile as well :/

#Step 1: Create pills_population_county_MD object 

#pills_population_county_MD <- pills_population %>%
...
#geo_pills_population_county_MD <- county_geodata%>%
#inner_join(pills_population_county_MD, by=c("GEOID" = "countyfips"))


#Step 3: Map it

#geo_pills_population_county_MD %>%
  #ggplot(aes(fill = pills_per_person)) +
  #facet_wrap(~year, nrow=2) +
  #geom_sf(lwd = 0) +
  #theme_map() +
  #labs(fill='Pills per Person',title="Md. Opioid Distribution Peaks in 2011", subtitle = "Baltimore and PG Counties hit the hardest", caption = "Source: ARCOS database") +
  #theme(legend.position="bottom") +
 # scale_fill_viridis_c(option = "cividis",labels = comma)
