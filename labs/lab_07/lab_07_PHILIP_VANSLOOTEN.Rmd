...
#Note: map opened in a new web page

#TRENDS: Based on the map, not only were counties in the southeast, such as Mingo, WV and a county in what appeared to be Florida, flooded with opioids in 2009 (the "opioid corridor"), but the west around California to about Colorado looked impacted as well compared to the midwest and northeast regions. I wonder if there is a correlation with the current homeless crisis in that area.
```
...
geo_pills_population_county_MD %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="Md. Opioid Distribution Peaks in 2010", subtitle = "Allegany, Cecil, Kent counties hit the hardest", caption = "Source: ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "cividis",labels = comma)
