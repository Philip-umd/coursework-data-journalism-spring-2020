...
#install arcos package [no need to install others, just here for demonstration]
...
# Step 1: the question - How did the total number of pill ssent to Mingo County, West Virginia each year change between 2006 and 2012?

#Step 2: Create a mingo_county_pills_per_year subset of the arcos_county_pills_per_year. 

mingo_county_pills_per_year<-arcos_county_pills_per_year %>%
  filter(buyer_state == "WV", buyer_county == "MINGO") %>%
  select(year, dosage_unit)

#Answer: In Mingo County, opioids surged in 2006, dropped in 2007, then rose until 2013.

#Step 3: Create a bar chart to visualize the answer.

ggplot(baltimore_city_pills_per_year) +
  geom_bar(stat="identity", aes(year, dosage_unit), fill="royal blue") +
  labs(x="Year", y="Total pills", title="In Mingo County, opioids surged in 2006, dropped in 2007, rose until 2013", subtitle = "Total pills shipped to Mingo County by year", caption = "Source: DEA ARCOS database, via Washington Post") +
  scale_x_continuous(breaks = c(2006, 2007, 2008, 2009, 2010, 2011, 2012, 2013, 2014)) +
  scale_y_continuous(labels = comma)

#Step 4: Save a web version of the chart.
...
```{r}
# Step 1: Ask a question - What counties in West Virginia had a disproportionately large number of opioids shipped relative to their populaiton size in 2012?

#Step 2: Create table 1 - West Virginia counties; pills received; for 2012.

west_virginia_pills_2012 <- arcos_county_pills_per_year %>%
  filter(buyer_state == "WV", year=="2012") %>%
  select(buyer_county, year, dosage_unit)

#Step 3: Create table 2 -- WV counties; population data; 2012
...
#Step 4: Create a combined table using the inner join function.

west_virginia_2012<- west_virginia_pills_2012 %>%
  inner_join(west_virginia_population_2012, by=("buyer_county")) %>%
  select(buyer_county, dosage_unit, population)

#ANSWER:Raleigh and Logan counties, WV had high numbers of opioids relative to populations in 2012

#Step 5: Create scatterplot chart.

ggplot(west_virginia_2012) +
  geom_point(aes(population, dosage_unit)) +
  labs(x="2012 Population", y="Total Pills in 2012", title="Raleigh and Logan counties, WV had high numbers of opioids relative to populations in 2012", caption = "Source: DEA ARCOS database, via Washington Post", fill="buyer_county") +
  scale_y_continuous(labels = comma) +
  scale_x_continuous(labels = comma) + 
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  geom_smooth(aes(population, dosage_unit), method = "lm", se = FALSE) +
