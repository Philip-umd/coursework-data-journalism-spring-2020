...
#1. Install the suite of packages below:
...
# 2. load the suite of packages below:
library(tidyverse)
library(janitor)
library(arcos)
library(tidycensus)
...
pills_population_income %>%
 ...
 group_by(quintile) %>%
 summarise(average_pills_per_person = mean(average_pills_per_person),
...
#Question: Did lower income areas tend to get flooded with pills more than wealthier areas? 

#Observations: 

#1 - Buckets: There seems to be a pattern here. The fifth quintile has the counties in the highest income bracket but the lowest average of pills per person shipped to them.  
#2 - Scatterplot: The slope of the trend line is down, indicating a possible negative correlation. So, in general, as a community's income bracket moved above $75,000, it saw fewer opioid shipments. The dots are pretty densely packed at the lower end of the pill shipments, so in general those receiving more than 100,000 pills were true outliers and in need of more reporting. 
#3 - Test the relationship: The r is -30, indicating a very mild negative relationship (closer to zero than -1). But the p value is 2.2e-16, which is smaller than .05 and is a significant result. So there is a mild but significant negative correlation here between income level and pill shipments. 

#Answer to question: I wouldn't necessarily write an article on from this angle because of the mild significance -- however, I feel the outliers are skewing the results here. I would feel fairly confident reporting the richer the neighborhood, the few the pills shipped, but I would focus my reporting budget on those outliers. That's more of a story. This result could be interesting context. 
