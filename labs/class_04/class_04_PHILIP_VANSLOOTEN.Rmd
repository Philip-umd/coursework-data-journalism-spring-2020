...
```{r}
# Open and view sheet in a new window tab for examination.
#View(baltimore)
```
...
```{r}
# Using glimpse  to examine the shape of the data.
glimpse(baltimore)

# there are 290,908 observations (rows) amd 42 variables (columns)

#Do you see any examples of a column stored as a "dbl" that doesn't really need to be, because we would never do math on it?  --> yes, zip codes, drug codes, NDC numbers, other descriptive variables

# Are there any columns stored as character that you think should be stored in another format entirely? --> possibly the null values if they should be numbers where math could be done

#Based on the information here, how are missing values represented in this data set?--> possibly as null values 

```

```{r}
#Using summary to examine the data.
summary(baltimore)
```
...
```{r}
#Use the clean names function to convert column names to lower case.
baltimore<-clean_names(baltimore)
```

```{r}
#Using the glimpse function to look at the results of the cleaning.
glimpse(baltimore)
```
...
```{r}
# Which single pharmacy **location** had the most pills sent to it? 
baltimore %>%
  group_by(buyer_name, buyer_dea_no, buyer_address1, buyer_city, buyer_state) %>%
  summarise(shipments = n(), total_pills = sum(dosage_unit)) %>%
  arrange(desc(total_pills))

# Answer: Newcare Home Health Services --> total_pills = 6,133,600

# What actions did the DEA and state of Maryland take against this pharmacy? Why did they do it? 

# write a paragraph (two to three sentences) that you could drop into a news story that includes the following information: Name and approximate location of the pharmacy; how many pills it received between 2006 and 2012; and a brief description of what actions the DEA and Maryland took against the pharmacy and why. 

# NewCare Home Health Services in Baltimore, Md. received 6,133,600 pills between 2006 and 2012, and the pharmacy owners were charged for illegally selling pills over the internet. 
...
# What is the name of the distributor that was responsible for sending the most pills to Baltimore City betwen 2006 and 2012? 
baltimore %>%
  group_by(reporter_name, reporter_state) %>%
  summarise(shipments = n(), total_pills = sum(dosage_unit)) %>%
  arrange(desc(total_pills))

# the name of the distributor --> MCKESSON CORPORATION; the number of pills sent between 2006 and 2012 --> 49,834,830; the state where their corporate headquarters is located--> MD.
