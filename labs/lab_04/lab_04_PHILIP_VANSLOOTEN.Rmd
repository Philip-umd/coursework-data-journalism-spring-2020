...
# Install Tidyverse package [completed previously]
...
# build as complete an address as possible out of the distributor information

mingo_working <- mingo %>%
  mutate(reporter_address_complete = paste0(reporter_address1,", ", reporter_city,", ", reporter_county, " COUNTY, ", reporter_state, " ", reporter_zip))    
...
# create a new column called "state_distribution" with values "in_state_distribution" or "out_of_state_distribution"
...
# the number of pills for each manufacturer / distributor / pharmacy grouping in Mingo County, WV
mingo %>%
  group_by(combined_labeler_name, reporter_name, buyer_name)%>%
  summarise(pills_per_pipeline = sum(dosage_unit)) %>%
  arrange(desc(pills_per_pipeline))
  
# The most significant combination of manufacturer / distributor / pharmacy pill shipments to Mingo County, WV was Actavis Pharma Inc. to Miami-Luken to Tug Valley Pharmacy LLC at 4,954,200 pills.
...
# Cardinal Health appears as CARDINAL HEALTH; CARDINAL HEALTH, INC.; and CARDINAL HEALTH 110, LLC.-- is this the same company? 
mingo %>%
  group_by(reporter_name, reporter_dea_no, reporter_address1, reporter_county, reporter_city, reporter_state, reporter_zip)%>%
  summarise(shipments = n()) %>%
  arrange(reporter_name)

# Each has a different DEA number, county and address info. 
...
----------------------
1. It seems like you need an extra column to tell you if a pharmacy is owned by Cardinal Health or not (binary seems the easiest fix).
     
   “owned_by_Cardinal_Health” or
   “   “ [blank] 

2. A carrot ^ hopefully can locate values with Cardinal at the beginning (fingers crossed).

   "^Cardinal"

3. This seems to me like a type of mutate() function:


4. Example: create a new column called "Cardinal_Health_Pharmacy" with values "owned_by_Cardinal Health" or "  "

mingo_working<-mingo %>% 
  mutate(Cardinal_Health_Pharmacy = case_when(
   Buyer_name == "^Cardinal" ~ "owned_by_Cardinal_Health",
    TRUE ~ "  "))
