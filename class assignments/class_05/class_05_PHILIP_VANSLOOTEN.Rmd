...
```{r}
#Load data and store it as an object called buyer_addresses
buyer_addresses<-read_csv("data/buyer_addresses.csv")

#Load data and store it as an object called buyer_annual_by_year
buyer_annual_by_year<- read_csv("data/buyer_annual_by_year.csv")

#Load data and store it as an object called buyer_totals
buyer_totals<- read_csv("data/buyer_totals.csv")

#Load data and store it as an object called state_population_per_year
...
#The three data sets describe similar things -- buyers -- but have different numbers of records. Why? --> different joins pulling different matching data -- the larger sets are closer to the original in data pulled (the larger set is probably a outer join and the smaller is an inner join)

#What's your best guess for why the number of records buyer_annual_by_year is higher than buyer_totals?  --> buyer totals pulled few matching records like an inner join

#What about your best guess for why buyer_addresses is higher than buyer_totals? buyers addresses is probably an outer or left/right join while totals is proably an inner join) 
...
#Question 1: What is the name and location of the pharmacy that had the most pills sent to it in total? 

#[an inner join of buyer_totals to buyer_addresses]

pills_by_pharmacy<-buyer_addresses %>%
  inner_join(buyer_totals, by="buyer_dea_no")

#Answer 1: VA CONSOLIDATED MAIL OUTPATIENT PHARMACY; 512,414,034 pills total 

#Question 2: Why does this pharmacy have so many? 

#Answer 2: The Post and Courier reported in July 18, 2019 reported this pharmacy shipped an increasing number of opioids to vets from 2006-12 (so pills didn't just go to local residents).
...
#Question 1: What is the name of the practitioner in Maryland that had the most totals pills **in 2010**?

#MD_total_pills_2010<-buyer_addresses %>%
  #inner_join(buyer_annual_by_year, by="buyer_dea_no") %>%
  #filter(year==as_date("2010"))

  

#Answer 1:

# How many total pills did the doctor have, compared to the next highest doctor in the state that year? 

#Use the Maryland state physician board lookup tool to find any disciplinary actions taken against this doctor: https://www.mbp.state.md.us/bpqapp/. Do a brief writeup of what you find.  

#Also answer this question: how would you ensure that the doctor you find on the state board lookup tool is the same doctor described in this data.  
