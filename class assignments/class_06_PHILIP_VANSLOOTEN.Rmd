...
# Load the packages.  You only need to install first if you've never loaded them on this machine.
 ...
 # Step 1 - install packages (that aren't all ready there)
...
# Build a bar chart that shows the total number of pills shipped to Maryland (NOT the number of shipments) each year between 2006 and 2014
...
maryland_pills_per_year <- arcos_county_pills_per_year %>%
  ...
  ggplot(maryland_pills_per_year)+
    ...
    labs(x="Year", y="Total Pills", title="In Maryland, opioids increased until 2011 then decreased to 2014", subtitle="Total pills shipped to Maryland by year", caption="Source: DEA ARCOS database, via Washington Post")+
...
```{r}
#Question: How does Maryland compare to Baltimore City year-by-year? 
#Answer: Baltimore City had a spike in 2006.
...
by_population_pills_2012<- county_population(key = key) %>%
  clean_names() %>%
  filter(year=="2012") %>%
...
#Step 3 -- join them 
  ...
  state_and_population_2012 <- by_state_pills_2012%>%
  left_join(by_population_pills_2012, by=("buyer_state")) %>%
  select(buyer_state, year, dosage_unit, population)
  
#Step 4 -- now chart (scatterplot)
  
  #ggplot(state_and_population_2012)+
   # geom_point(aes(population, dosage_unit))+
    # labs(x="2012 Population", y="Total pills 2012", title="States, population and total opioids in 2012", caption = "Source: DEA ARCOS database, via Washington Post", fill="buyer_state") +
  #scale_y_continuous(labels = comma) +
  ...
  #geom_text_repel(aes(population, dosage_unit, label=buyer_state)), 
    #subset(state_and_population_2012, population>175000)
