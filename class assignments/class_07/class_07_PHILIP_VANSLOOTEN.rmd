...
#QUESTION: Shouldn't we do an anti-join here to capture missing records from each table? If not, when are cases when we should do an anti-join?
```
...
geo_pills_population_county_WV %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="WV. Opioid Impact Peaks in 2008", subtitle = "Mingo County hit the hardest; Logan struggles longest with recovery", caption = "Source: ARCOS database") +
...
```{r}
#WV 2006-09 MAP
#Step 1: WV table adjusted for only less than 2009

pills_population_county_WV_0609<- pills_population %>%
...
# Step 2: Join ADJUSTED WV table to a county level shapefile
geo_pills_population_county_WV_0609 <- county_geodata%>%
inner_join(pills_population_county_WV_0609, by=c("GEOID" = "countyfips"))

#Step 3: ADJUSTED WV County map
geo_pills_population_county_WV_0609 %>%
  ggplot(aes(fill = pills_per_person)) +
...
# Step 1: Filter the pills population table just for Kentucky and create the pills per person metric.  
pills_population_county_KY <- pills_population %>%
...
geo_pills_population_county_KY <- county_geodata%>%
inner_join(pills_population_county_KY, by=c("GEOID" = "countyfips"))

#Step 3: KY table adjusted for only less than 2009

pills_population_county_KY_0609<- pills_population %>%
  filter(buyer_state=='KY', year<=2009) %>%
  group_by(countyfips, buyer_county, year) %>%
  summarise(total_pills=sum(dosage_unit, na.rm = TRUE),
           total_population=sum(population, na.rm=TRUE)) %>%
  mutate(pills_per_person=total_pills/total_population)

# Step 4: Join ADJUSTED KY table to a county level shapefile
geo_pills_population_county_KY_0609 <- county_geodata%>%
inner_join(pills_population_county_KY_0609, by=c("GEOID" = "countyfips"))

#Step 5: ADJUSTED KY County map
geo_pills_population_county_KY_0609 %>%
  ggplot(aes(fill = pills_per_person)) +
  facet_wrap(~year, nrow=1) +
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="Southeastern Kentucky Devastated by Opioid Crisis", subtitle = "Whitley County Flooded from '06-'09", caption = "Source: ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "cividis",labels = comma)
